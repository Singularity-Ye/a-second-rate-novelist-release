export type OpeningId = "zifu" | "xueying" | "immortal" | "martial" | "mystery" | "court";
...
export const OPENING_CHOICES: OpeningChoice[] = [
  {
    id: "zifu",
    title: "因果放行",
    scene: "你替宿敌查一笔因果，查到第四层时，先被自己的真名叫醒。",
    genre: "修仙因果悬疑",
  },
  {
    id: "xueying",
    title: "血影成真",
    scene: "你明明不会血影遁，却在濒死时喊了出来；下一息，系统替你把谎话写进现实。",
    genre: "搞笑系统修仙",
  },
];

// Kept for local fixtures and persisted drafts only. The formal entrance above
// deliberately exposes no legacy opening cards.
const LEGACY_OPENING_CHOICES: readonly OpeningChoice[] = [
  {
    id: "immortal",
    title: "破庙残剑",
...
const OPENING_COPY: Record<OpeningId, Record<ToneId, string>> = {
  zifu: {
    rise: "你替宿敌查到第四层因果，先被一声自己的真名叫醒。醒来后，所有证据都指向你最不该相信的人。",
    abyss: "宿敌的因果账本只剩一页空白，而那一页写着你的名字。你翻到背面，发现自己的来路也被人删过。",
    secret: "你没有立刻放行这笔因果，只把第四层线索藏进袖口。可城中每个人，忽然都开始叫你另一个名字。",
  },
  xueying: {
    rise: "你喊出血影遁时其实只想吓退追兵，系统却认真替你把谎话写进现实，下一息整条街都开始追问你师承何处。",
    abyss: "濒死之际你随口喊出血影遁，影子真的把你拖走了。系统在耳边补充：检测到新技能，暂时无法解释。",
    secret: "你明明不会血影遁，却在众目睽睽下喊了出来。没人知道是你在说谎，还是现实先学会了配合。",
  },
  immortal: {
...
export function createOpeningWorld(openingId: OpeningId, toneId: ToneId): WorldRecord {
  const opening = [...OPENING_CHOICES, ...LEGACY_OPENING_CHOICES].find((item) => item.id === openingId) ?? OPENING_CHOICES[0]!;
  const tone = TONE_CHOICES.find((item) => item.id === toneId) ?? TONE_CHOICES[0]!;
  const stableMetadata = opening.id === "zifu"
    ? {
        storyId: "story-zifu-zhangwairen-001",
  ...
  const protagonist = opening.id === "court" || opening.id === "xueying"
    ...
    : opening.id === "mystery"
      ? "周既明"
      : "陆停舟";
  const anchorPlace = opening.id === "mystery"
    ? "北岸旧站"
    : opening.id === "court" || opening.id === "xueying"
      ? "含章殿"
      : opening.id === "martial"
        ? "赤鲸擂台"
        : "听雨破庙";
  const faction = opening.id === "court" || opening.id === "xueying"
    ? "司夜台"
    : opening.id === "mystery"
      ? "第七码头分局"
      : opening.id === "martial"
        ? "临海武馆"
        : "照骨山";
  const object = opening.id === "mystery"
    ? "明日车票"
    : opening.id === "court" || opening.id === "xueying"
      ? "空白密诏"
      : opening.id === "martial"

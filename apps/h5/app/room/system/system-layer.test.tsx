...
function stubActiveGateway(
  ...
  vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL) => {
    if (String(input).endsWith("/api/vnext/experience")) return projectionResponse();
    return roomRequest();
  }));
}
...
async function waitForActiveGateway() {
  await waitFor(() => {
    expect(screen.getByTestId("system-task").textContent).toContain("小韩在这里");
  });
}
...
describe("SystemLayerPanel", () => {
  ...
  it("shows conversation first while keeping task and evidence in internal records", async () => {
    render(
      <SystemLayerPanel
        observation={{ sceneLabel: "书房", activityLabel: "study-writing", focus: 72, fatigue: 20, inspiration: 48, emotionalLoad: 18 }}
      />,
  ...
  it("opens the two chat channels with keyboard shortcuts and keeps escape local to the panel", async () => {
    render(
      <SystemLayerPanel
        observation={{ sceneLabel: "书房", activityLabel: "study-writing", focus: 72, fatigue: 20, inspiration: 48, emotionalLoad: 18 }}
      />,
    ...
    expect(screen.getByTestId("system-layer-panel").getAttribute("data-chat-mode")).toBe("subsystem");
    ...
    expect(screen.getByTestId("system-layer-panel").getAttribute("data-expanded")).toBe("false");
  });
  ...
  it("renders a real SSE response as online streaming instead of a local rule reply", async () => {
    stubActiveGateway(async () => new Response(
      ...
      + "event: ready\ndata: {\"type\":\"ready\",\"requestId\":\"room-request-1\",\"provider\":\"company-router\",\"model\":\"grok-4.5\",\"requestedTier\":\"light\",\"routeFallbackApplied\":false,\"fallbackApplied\":false}\n\n"
      + "event: chunk\ndata: {\"type\":\"chunk\",\"text\":\"这句来自流式接口。\"}\n\n"
      + "event: complete\ndata: {\"type\":\"complete\",\"requestId\":\"room-request-1\",\"provider\":\"company-router\",\"model\":\"grok-4.5\",\"requestedTier\":\"light\",\"routeFallbackApplied\":false,\"fallbackApplied\":false}\n\n",

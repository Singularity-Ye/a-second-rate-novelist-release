...
const MANAGED_DATABASE_PATTERN = /^erliu_tc164_worker_[0-9a-f]{24}$/;
...
function createManagedDatabase(
  ...
  const name = `erliu_tc164_worker_${randomBytes(12).toString("hex")}`;
  command(
    "createdb",
  ...
  console.log(`[TC164] created managed disposable database ${name}`);
...
function dropManagedDatabase(
  ...
  console.log(`[TC164] dropped and verified managed database ${database.name}`);
}
...
function liveEnv(database: DisposablePostgresDatabase) {
  return {
    ...
    TC164_DATABASE_URL: database.url,
    TC164_POSTGRES_GATE: "1",
  };
}
...
function verifyMigrationHistory(
  ...
  console.log(`[TC164] canonical migration history ${canonical.length}/${canonical.length}`);
}

function runGate(database: DisposablePostgresDatabase) {
  const env = liveEnv(database);
  console.log("[TC164] build shared contract, generate and validate Prisma");
  ...
  console.log("[TC164] deploy canonical migrations");
  runPrisma(["migrate", "deploy", "--schema", schemaPath], database);
  runPrisma(["migrate", "status", "--schema", schemaPath], database);

  console.log("[TC164] execute worker state, repository and independent-process evidence");
  command(
    "pnpm",
    [
      "exec",
      ...
      "../../tests/integration/vnext/prisma-creative-task.repository.spec.ts",
      "../../tests/integration/vnext/vnext-worker-postgres-runtime.spec.ts",
      "--maxWorkers=1",
  ...
  command("pnpm", ["verify:vnext-boundaries"], { cwd: workspaceDir, env });
  runPrisma(["migrate", "status", "--schema", schemaPath], database);
  ...
  console.log(`[TC164] full-schema zero diff (${diff.trim()})`);
}

async function main() {
  const admin = requireLocalPostgresAdminDatabase("TC164_ADMIN_DATABASE_URL");
  ...
  try {
    ...
    console.log(
      "[TC164] PASS: code_audit + runtime_worker_synthetic (provider runtime not claimed)",
    );
  } catch (error) {
    gateFailure = error;
  } finally {
      ...
      try {
        ...
        if (gateFailure) {
          throw new AggregateError(
            ...
            "TC164 gate and exact cleanup both failed",
          );
        }
        throw cleanupError;
      }

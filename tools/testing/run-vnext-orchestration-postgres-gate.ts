...
const MANAGED_DATABASE_PATTERN =
  /^erliu_tc165_orchestration_[0-9a-f]{24}$/;
...
const tc165Specs = [
  "../../tests/integration/vnext/vnext-creative-uow-failure.spec.ts",
  "../../tests/integration/vnext/vnext-experience-projection-postgres.spec.ts",
  "../../tests/integration/vnext/vnext-experience-draft-postgres.spec.ts",
...
function createManagedDatabase(
  ...
  const name = `erliu_tc165_orchestration_${randomBytes(12).toString("hex")}`;
  ...
  console.log(`[TC165] created managed disposable database ${name}`);
...
function dropManagedDatabase(
  ...
  console.log(
    `[TC165] dropped and verified managed database ${database.name} (count=0)`,
  );
}
...
function liveEnv(database: DisposablePostgresDatabase) {
  return {
    ...
    TC165_DATABASE_URL: database.url,
    TC165_POSTGRES_GATE: "1",
  };
}
...
function verifyMigrationHistory(
  ...
  console.log(
    `[TC165] canonical migration history/checksums ${canonical.length}/${canonical.length}`,
  );
}
...
function runGate(database: DisposablePostgresDatabase) {
  ...
  console.log("[TC165] build shared contract, generate and validate Prisma");
  ...
  console.log("[TC165] deploy and verify all canonical migrations");
  runPrisma(["migrate", "deploy", "--schema", schemaPath], database);
  runPrisma(["migrate", "status", "--schema", schemaPath], database);

  console.log(
    "[TC165] execute UoW, projection, HTTP, adapter, and orchestration evidence serially",
  );
  for (const spec of tc165Specs) {
    console.log(`[TC165] vitest ${path.basename(spec)}`);
    command(
      "pnpm",
      [
        "exec",
  ...
  console.log(`[TC165] full-schema zero diff (${diff.trim()})`);
}

function main() {
  const admin = requireLocalPostgresAdminDatabase("TC165_ADMIN_DATABASE_URL");
      ...
      try {
        ...
        if (gateFailure) {
          throw new AggregateError(
            ...
            "TC165 gate and exact cleanup both failed",
          );
        }
        throw cleanupError;
      }
  ...
  if (createdNames.size !== 0) {
    throw new Error(
      `TC165 cleanup left ${createdNames.size} tracked managed database(s)`,
    );
  }
  if (gateFailure) {
    throw gateFailure;
  ...
  console.log(
    "[TC165] PASS: code_audit + local_postgresql (provider runtime not claimed)",
  );
}
